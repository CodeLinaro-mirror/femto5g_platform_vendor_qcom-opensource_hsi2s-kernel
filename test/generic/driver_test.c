...
int main(int argc, char **argv)
{
	...
	uint32_t buffer_ms;
	uint32_t bit_depth;
	uint8_t spkr_channel_count;
	uint8_t mic_channel_count;
	uint32_t long_rate = 0;
	uint8_t pcm_rate;
	uint8_t pcm_sync_src;
	uint8_t pcm_aux_mode;
	uint8_t pcm_tpcm_width;
	uint8_t pcm_rpcm_width;
	uint8_t tdm_sync_delay;
	uint32_t tdm_tpcm_width;
	uint32_t tdm_rpcm_width;
	uint32_t tdm_rate;
